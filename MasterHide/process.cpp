...
NTSTATUS Initialize()
{
    ...
    NTSTATUS status = g_processResource.Initialize();
...
void Deinitialize()
{
    ...
    WppTracePrint(TRACE_LEVEL_VERBOSE, GENERAL, "Successfully de-initialized process interface!");
    return;
}
...
bool ClearPebNtGlobalFlag(PEPROCESS process)
{
    ...
    if (Peb32)
    {
        ...
        {
            KeStackAttachProcess(process, &apcState);
            ClearFlag(Peb32->NtGlobalFlag, NT_GLOBAL_FLAG_DEBUGGED);
            ClearFlag(Peb->NtGlobalFlag, NT_GLOBAL_FLAG_DEBUGGED);
        }
        __finally
        {
            KeUnstackDetachProcess(&apcState);
...
bool ClearBypassProcessFreezeFlag(_In_ PPROCESS_ENTRY processEntry)
{
    ...
    for (PSYSTEM_PROCESS_INFORMATION entry = processInfo; entry->NextEntryOffset != NULL;
         entry = reinterpret_cast<PSYSTEM_PROCESS_INFORMATION>((PUCHAR)entry + entry->NextEntryOffset))
    {
...
bool ClearThreadHideFromDebuggerFlag(_In_ PPROCESS_ENTRY processEntry)
{
    ...
    const NTSTATUS status =
        tools::QuerySystemInformation(SystemProcessInformation, reinterpret_cast<PVOID *>(&processInfo));
    if (!NT_SUCCESS(status))
    {
        WppTracePrint(TRACE_LEVEL_ERROR, GENERAL, "ZwQuerySystemInformation returned %!STATUS!", status);
        return false;
...
static ULONG_PTR IpiFlushTbCallback(_In_opt_ ULONG_PTR argument)
{
...
void HookKuserSharedData(_In_ PPROCESS_ENTRY processEntry)
{
    ...
    PVOID NewKuserSharedData = MmAllocateContiguousMemory(sizeof(KUSER_SHARED_DATA), highestAddr);
...
void UnHookKuserSharedData(_In_ PPROCESS_ENTRY processEntry)
{
    PAGED_CODE();
    NT_ASSERT(processEntry);

    ClearFlag(processEntry->PolicyFlags, ProcessPolicyFlagHideKUserSharedData);
...
void CounterUpdater(PVOID Context)
{
    ...
    DBGPRINT("Starting counter updater thread");
    ...
    while (!g_stopCounterThread)
    {
        ...
        if (g_processResource.LockExclusive())
        {
            EnumProcessesUnsafe([](_In_ PPROCESS_ENTRY processEntry) -> bool {
    ...
    DBGPRINT("Leaving counter updater thread");
...
PTHREAD_ENTRY PROCESS_ENTRY::AppendThreadList(_In_ PETHREAD thread)
{
    ...
    if (ThreadsResource.LockExclusive())
    {
        SCOPE_EXIT
        {
        ...
        //
        threadEntry =
            tools::AllocatePoolZero<PTHREAD_ENTRY>(NonPagedPool, sizeof(THREAD_ENTRY), tags::TAG_THREAD_ENTRY);
        if (!threadEntry)
        {
...
void Deinitialize()
{
    ...
    WppTracePrint(TRACE_LEVEL_VERBOSE, GENERAL, "Successfully de-initialized process rules interface!");
    return;
}
