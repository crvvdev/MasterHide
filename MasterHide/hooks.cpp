...
{
    const auto ret = oNtDeviceIoControlFile(FileHandle, Event, ApcRoutine, ApcContext, IoStatusBlock, IoControlCode,
                    ...
                    if (OutputBufferLength >= sizeof(STORAGE_DEVICE_DESCRIPTOR))
                    {
                        ...
                        if (Desc)
                        {
                            if (Desc->SerialNumberOffset)
                            {
                                ...
                                DBGPRINT("%ws Spoofing Serial ( 0x%X ) Old: %s New: %s\n", ShortName, IoControlCode,
                                         Serial, globals::szFakeSerial);
