...
namespace Tools
{
	...
	ULONG64 FindPattern( const ULONG64 base, const ULONG size, const char* bmask, const char* szmask )
	{
	...
	NTSTATUS LoadFile( PUNICODE_STRING FileName, PVOID* pImageBase )
	{
		if ( !FileName )
	...
	ULONG GetNtSyscall( LPCSTR FunctionName )
	{
		...
		if ( pNTDLL )
		{
			auto Fn = PUCHAR( GetFunctionAddress( pNTDLL, FunctionName ) );
			if ( Fn )
			{
				for ( int i = 0; i < 24; ++i )
				{
	...
	ULONG GetWin32Syscall( LPCSTR FunctionName )
	{
		...
		if ( pWin32u )
		{
			auto Fn = PUCHAR( GetFunctionAddress( pWin32u, FunctionName ) );
			if ( Fn )
			{
				for ( int i = 0; i < 24; ++i )
				{
