...
inline void Load()
{
    ...
    {
        throw std::runtime_error("Failed to obtain system directory!");
    }

    const auto path = std::string{buf} + "\\drivers\\klhk.sys";

    g_serviceHandle = service::CreateOrOpen("klhk", "Kaspersky Lab service driver", path);
    if (!g_serviceHandle)
    {
...
inline void Cleanup(bool deleteService)
{
    if (g_parametersKey)
    {
        RegCloseKey(g_parametersKey);
